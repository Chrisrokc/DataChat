...
public class OpenAiChatService : IAiChatService
{
    ...
    {
        var (client, model, options) = await GetClientAndOptionsAsync(cancellationToken);
    ...
    private async Task<(ChatClient client, string model, ChatCompletionOptions options)> GetClientAndOptionsAsync(
        CancellationToken cancellationToken)
    {
        ...
        var chatClient = openAiClient.GetChatClient(config.OpenAiModel);

        var options = new ChatCompletionOptions
        {
            MaxOutputTokenCount = config.MaxTokensPerRequest,
            Temperature = (float)config.Temperature
        };

        return (chatClient, config.OpenAiModel, options);
    }
    ...
    private static string ExtractExcelText(byte[] bytes, string fileName)
    {
        try
        {
            ...
            foreach (var worksheet in workbook.Worksheets)
            {
                ...
                var lastRow = Math.Min(usedRange.LastRow().RowNumber(), firstRow + 499); // Limit to 500 rows per sheet for chat
    ...
    private static List<byte[]> RenderPdfPagesToImages(byte[] pdfBytes)
    {
        ...
        {
            // Render page to SKBitmap using PDFtoImage
